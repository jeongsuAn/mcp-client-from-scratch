...
print(resp.headers)  # 빈 헤더
print(resp.content.decode())  # 빈 문자열
...
ollama_tools = [convert_mcp_tool_to_openai(tool) for tool in tools_list]
print(f"Ollama에 {len(ollama_tools)}개의 도구를 전달합니다: {[t['function']['name'] for t in ollama_tools]}")
...
try:
    ...
    # 응답 구조가 다름: `response.choices[0].message` -> `response['message']`
    ...
    if tool_calls:
        ...
        # 9. 두 번째 llama 호출 (도구 결과와 함께)
        print("도구 실행 결과를 llama3.1에 전달하여 최종 답변을 생성합니다.")
